...
const buttonVariants = cva('cursor-pointer flex items-center justify-center', {
  variants: {
    variant: {
      ...
      outline:
        '[&>*]:text-secondary-fg text-secondary-fg hover:bg-secondary shadow-[inset_0_0_0_1px_var(--color-stroke-trivial)] bg-bg [background-image:radial-gradient(90%_80%_at_50%_40%,var(--color-bg)_40%,--alpha(var(--color-secondary-fg)/20%))]',
...
const ButtonLink = forwardRef<HTMLAnchorElement, ButtonLinkProps>(function ButtonLink(
  { className, variant, size, isDisabled = false, ...props },
  ref
) {
  return (
    <Link
